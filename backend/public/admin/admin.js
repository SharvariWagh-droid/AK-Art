...
async function loadArtworks() {
  ...
  const res = await fetch(`/api/artworks?type=${type}`);
...
async function loadAboutStyle() {
  try {
    const res = await fetch("/api/about/style");
...
async function loadTestimonialStyles() {
  try {
    const res = await fetch("/api/testimonials/style");
    const style = await res.json();
...
async function saveHomepageCMS(e) {
  ...
  try {
    ...
    if (hasNewFiles) {
      const uploadRes = await fetch("/api/homepage/upload", {
        method: "POST",
        body: formData
      });
    ...
    const updateRes = await fetch("/api/homepage", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
